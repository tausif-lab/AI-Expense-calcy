...
function sanitizeTool(
  ...
  const officialPrice = prices[raw.name]?.[raw.plan] ?? undefined;
...
// ─────────────────────────────────────────────
function inferActualPlan(
  tool: ToolInput,
  prices: Record<string, Record<string, number>>,
): string {
  const spendPerSeat =
    tool.seats > 0 ? tool.monthlySpend / tool.seats : tool.monthlySpend;
  const toolPrices = prices[tool.name];
  if (!toolPrices) return tool.plan;
  ...
  if (tool.plan === "Enterprise" || tool.plan === "Business") {
    ...
    for (const [plan, price] of Object.entries(toolPrices)) {
      if (price === 0) continue;
...
export function runAuditEngine(
  ...
  const prices = pricingData ?? OFFICIAL_PRICES;
  const sanitizedTools = input.tools.map((t) => sanitizeTool(t, prices));
  const sanitizedInput: FormInput = { ...input, tools: sanitizedTools };

  const findings: ToolFinding[] = [];

  for (const tool of sanitizedTools) {
    const finding = analyzeTool(tool, sanitizedInput, prices);
...
function analyzeTool(
  ...
  const officialPricePerSeat = prices[tool.name]?.[tool.plan] ?? 0;
  const inferredPlan = inferActualPlan(tool, prices);
  const inferredPrice =
    prices[tool.name]?.[inferredPlan] ?? officialPricePerSeat;
...
function analyzeEnterprisePlan(
  ...
  if (
    !justified &&
    tool.seats < 10 &&
    BASIC_SUFFICIENT_FEATURES.includes(tool.primaryFeatureUsed)
  ) {
    ...
    const midTierCost = midTier
      ? midTier.price * tool.seats
      : estimatedEnterpriseCost * 0.5;
    const saving = Math.max(
      0,
      Math.round(estimatedEnterpriseCost - midTierCost),
    );
  ...
  if (tool.monthlySpend > 0) {
    ...
    return {
      ...
      inferredSpend: tool.monthlySpend,
      recommendedAction:
        "Enterprise plan is justified for your scale and use case",
      estimatedMonthlySaving: 0,
      severity: "optimal",
...
function checkWrongPlanForFeature(
  ...
  if (
    ...
    return {
      ...
      plan: tool.plan,
      currentSpend: tool.monthlySpend,
      inferredSpend: 19.99 * tool.seats,
...
function checkWrongPlanForTeamSize(
  ...
  if (tool.name === "Cursor" && tool.plan === "Pro" && tool.seats >= 10) {
    return {
      toolName: tool.name,
      plan: tool.plan,
      currentSpend: tool.monthlySpend,
      inferredSpend: 40 * tool.seats,
  ...
  if (tool.name === "ChatGPT" && tool.plan === "Business" && tool.seats === 1) {
...
function checkCrossToolRedundancy(
  ...
  if (
    ...
    findings.push({
      ...
      recommendedAction:
        "Claude subscription + Anthropic API — consolidate to one billing method",
  ...
  if (
    ...
    findings.push({
      ...
      inferredSpend: sub.monthlySpend,
      recommendedAction:
        "ChatGPT subscription + OpenAI API — pick one, do not pay for both",
      estimatedMonthlySaving: sub.monthlySpend,
      severity: "medium",
  ...
  if (toolNames.includes("Cursor") && toolNames.includes("Windsurf")) {
    ...
    const cheaperTool =
      cursor.monthlySpend <= windsurf.monthlySpend ? cursor : windsurf;
    const expensiveTool = cheaperTool === cursor ? windsurf : cursor;
    findings.push({
      toolName: expensiveTool.name,
      plan: expensiveTool.plan,
      currentSpend: expensiveTool.monthlySpend,
      inferredSpend: 0,
  ...
  if (
    ...
    toolNames.includes("ChatGPT") &&
    tools
      .filter((t) => t.name === "Claude" || t.name === "ChatGPT")
    ...
    const pricier =
      claudeTool.monthlySpend >= chatTool.monthlySpend ? claudeTool : chatTool;
    findings.push({
      toolName: pricier.name,
      plan: pricier.plan,
      currentSpend: pricier.monthlySpend,
      inferredSpend: 0,
  ...
  if (toolNames.includes("GitHub Copilot") && toolNames.includes("Cursor")) {
    ...
    if (
      ...
      const cheaper =
        copilot.monthlySpend <= cursor.monthlySpend ? copilot : cursor;
      findings.push({
        toolName: cheaper.name,
        plan: cheaper.plan,
