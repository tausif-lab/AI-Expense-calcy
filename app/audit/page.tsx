...
const PLANS: Record<string, string[]> = {
  ...
  Default: ["Starter", "Pro", "Business", "Enterprise", "Usage-Based"],
};
...
export default function AuditForm() {
  ...
  const {
    ...
    defaultValues: {
      ...
      tools: [{ name: "Cursor", plan: "Pro", seats: 1, monthlySpend: 20, intensity: "Medium", usage: "Coding" }],
    },
    mode: "onChange",
  });
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                <header>
                  <h1 className="text-3xl font-bold tracking-tight">Company Context</h1>
                  <p className="text-gray-500 mt-2">Tell us about your team to help us benchmark your spend.</p>
                </header>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-gray-400">Total number of employees in your org.</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      {["Coding", "Writing", "Research", "Data Analysis", "Mixed"].map(opt => (
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold">Company Stage (Optional)</label>
                    <select 
                      {...register("companyStage")}
                      className="w-full p-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none bg-white transition-all"
                    >
                      <option value="">Select Stage</option>
                      {["Solo Founder", "Startup (2-10)", "Growing (11-50)", "Scale-up (50+)"].map(opt => (
                        <option key={opt} value={opt}>{opt}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold flex items-center gap-2">
                      <CreditCard className="w-4 h-4 text-gray-400" /> Monthly AI Budget
                    </label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        placeholder="e.g. 500"
                        className="w-full p-3 pl-7 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                      />
                    </div>
                  </div>
                </div>
            ...
            {step === 2 && (
              ...
              >
                <header className="flex justify-between items-end">
                  <div>
                    <h1 className="text-3xl font-bold tracking-tight">Your AI Stack</h1>
                    <p className="text-gray-500 mt-2">Add the tools and subscriptions your team uses daily.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => append({ name: "ChatGPT", plan: "Plus", seats: 1, monthlySpend: 20, intensity: "Medium", usage: "Mixed" })}
                  ...
                  {fields.map((field, index) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                          <label className="text-xs font-bold uppercase text-gray-400">Tool Name</label>
                          <select 
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-bold uppercase text-gray-400">Current Plan</label>
                          <select 
                            {...register(`tools.${index}.plan`)}
                            className="w-full p-2.5 rounded-lg border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                          >
                            {(PLANS[watchedTools[index]?.name] || PLANS.Default).map(opt => (
                              <option key={opt} value={opt}>{opt}</option>
                            ))}
                          </select>
                        </div>
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-bold uppercase text-gray-400">Seats</label>
                          <input 
                            type="number"
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-bold uppercase text-gray-400">Monthly Spend</label>
                          <div className="relative">
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-bold uppercase text-gray-400">Usage Intensity</label>
                          <select 
                            {...register(`tools.${index}.intensity`)}
                            className="w-full p-2.5 rounded-lg border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                          >
                            {["Light", "Medium", "Heavy"].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                          </select>
                        </div>

                        <div className="space-y-2">
                          <label className="text-xs font-bold uppercase text-gray-400">Main Usage</label>
                          <select 
                            {...register(`tools.${index}.usage`)}
                            className="w-full p-2.5 rounded-lg border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                          >
                            {["Coding", "Writing", "Research", "Analysis"].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                          </select>
                        </div>
                      </div>
                    </motion.div>
            ...
            {step === 3 && (
              ...
              >
                <header>
                  <h1 className="text-3xl font-bold tracking-tight">Review & Analyze</h1>
                  <p className="text-gray-500 mt-2">Double check your inputs before we generate your audit.</p>
                </header>

                <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm">
                  <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                    <div>
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Audit Summary</p>
                      <h3 className="text-xl font-bold mt-1">{watch("teamSize")} Team Members</h3>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Monthly Spend</p>
                      <h3 className="text-2xl font-bold text-emerald-600 mt-1">
                        ${watchedTools.reduce((acc, curr) => acc + (curr.monthlySpend || 0), 0)}
                      </h3>
                    </div>
                  </div>
                      ...
                      <div key={i} className="flex justify-between items-center text-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-xs">{tool.plan} • {tool.seats} seats</p>
                          </div>
                        </div>
                        <span className="font-semibold text-gray-900">${tool.monthlySpend}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 flex gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold text-emerald-900">Privacy Guaranteed</p>
                    <p className="text-sm text-emerald-700/80">Your data is only used to calculate savings. We never share your stack details with third parties.</p>
                  </div>
                </div>
              </motion.div>
            )}
